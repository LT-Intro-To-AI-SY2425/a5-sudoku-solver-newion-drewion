...
class Board:
    ...
    # Remove the assignment from other cells in the same row
        for c in range(self.size):
            if isinstance(self.rows[row][c], list):
                remove_if_exists(self.rows[row][c], assignment)

    # Remove the assignment from other cells in the same column
        for r in range(self.size):
            if isinstance(self.rows[r][column], list):
                remove_if_exists(self.rows[r][column], assignment)

    # Remove the assignment from other cells in the same subgrid
